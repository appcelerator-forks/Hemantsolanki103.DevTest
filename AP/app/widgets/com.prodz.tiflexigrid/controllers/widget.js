...
var init = function(opts) {
	...
	$.fgMain.top = params.top || 0;
	$.fgMain.bottom = params.bottom || 0;
	//ITEMS OPTIONS
	itemsOptions = {
	...
	addGridItems(data);
	consoleLog('data1',JSON.stringify(data));
};
...
var addGridItems = function(args) {
	...
	for (var x = 0; x < data.length; x++) {
		if (k % 2 == 0) {
			if (x <= data.length - 2) {
				addGridItem(data[x], false, true);
			} else {
			addGridItem(data[x], false, false);
			}

		} else {
			if (x < data.length - 2) {
				addGridItem(data[x], true, true);
			} else {
				addGridItem(data[x], true, false);
			}
...
var addGridItem = function(item, isSeeSeparator, isHorizontalSep) {
	...
	var sepVer = Ti.UI.createView({
		width : 1,
		height : Alloy.Globals.size_60,
		right : 0,
		backgroundColor : '#92ccc5'
	});
	var sepHori = Ti.UI.createView({
		width : Alloy.Globals.size_140,
		height : 1,
		right : 0,
		bottom : 0,
		backgroundColor : '#92ccc5'
